...
<body>
<!--  
	pageContext 객체는 JSP 기본 내장 객체, JSP페이지에 대한 1:1로 연결된 객체로
	 JSP페이지당 하나의 pageContext 객체가 생성된다.
	같은 페이지내에서 서로 값을 공유할 수 있으며, 
	주요기능은 다른 기본 객체들을 구할 때 사용하거나 페이지 흐름을 제어할 때 사용한다.
	 ...
	 ** 속성을 이용해서 데이터 저장 및 호출할 수 있다. (생명주기가 해당페이지를 벗어날시 소멸.)
	저장방법: pageContext.setAttribute("이름", 객체)
	호출방법: pageContext.getAttribute("이름");
	생명주기 : 해당페이지를 벗어날 시 저장정보도 소멸.
	...
	생명주기)application >  session > request >  pageContext
		 ...
		 session : 해당 브라우저가 종료되면 저장정보도 사라진다.
		 		(예외적으로 시간을 지정할 수 있다.)
 				로그인 정보처리는 무조건 session 정보에 저장해야한다.
 		application : 어플리케이션이 종료되기 전까지 정보는 사라지지않는다.
 					 어플리케이션 외부 환경정보(context)를 나타내는 내부객체이다.
 -->
